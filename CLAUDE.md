...
- **Renderer** (`src/renderer/`): a thin `@phlix/ui` consumer — no local pages/components/stores. Pinned to `@phlix/ui` (`github:detain/phlix-ui#v0.98.34`) + `@phlix/contracts` (`github:detain/phlix-contracts#v0.4.1`); Vue 3 / Pinia / vue-router are peer deps. Built with Vite + `@vitejs/plugin-vue`.
